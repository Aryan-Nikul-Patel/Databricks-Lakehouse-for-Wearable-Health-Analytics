...
# MAGIC %md
# MAGIC This class is responsible for loading raw data into the bronze layer of your data pipeline using Spark Structured Streaming. It watches folders in the landing zone and ingests:
# MAGIC
# MAGIC - User registration data (from CSV),
# MAGIC
# MAGIC - Gym login activity (from CSV),
# MAGIC
# MAGIC - Kafka messages like user_info, workout, and bpm (from JSON).
# MAGIC
# MAGIC Each stream adds metadata like load time and source file name. For Kafka data, it also joins with a date_lookup table to enrich records with date-related info like week_part. You can run the ingestion once (batch-like) or on a schedule (streaming), and the class provides validation methods to check if the expected number of records were loaded per topic/table.

...
# MAGIC %md
# MAGIC This class performs CDC (Change Data Capture) upserts during streaming. For each micro-batch, it filters out only the latest record per ID (based on a sort column like a timestamp), keeping only the most recent "new" or "update" entry. It creates a temporary view with those records and then runs a merge query to upsert the data into the target table.
...
# MAGIC %md
# MAGIC The Silver class is responsible for processing Bronze layer data and turning it into clean, deduplicated, and query-ready Silver tables. It handles various domains like user registrations, gym logs, workout sessions, and heart rate data. It uses Spark Structured Streaming with foreachBatch to apply upserts in a scalable, idempotent way.
# MAGIC - Upsert logic is used across all methods to ensure consistency and support reprocessing.
# MAGIC - Watermarking and dropDuplicates are applied to avoid duplicates and handle late-arriving data.
# MAGIC - Configurable triggers allow both batch-style (availableNow) and continuous streaming (processingTime).
# MAGIC - Checkpoints ensure fault-tolerance and allow resume from where the stream left off. 
# MAGIC - Uses a clean three-layer streaming design, processing in logical stages for clarity and performance.

# COMMAND ----------

# MAGIC %md
# MAGIC Method Summary
# MAGIC - upsert_users: Inserts new user registrations while ignoring re-registrations. Designed to be idempotent.
# MAGIC - upsert_gym_logs: Merges logins with updated logout times. Ensures that only newer logout values update the table.
# MAGIC - upsert_user_profile: Handles CDC from a Kafka topic, applies deduplication, and keeps only the latest update per user.
# MAGIC - upsert_workouts: Processes start/stop workout events, deduplicates by user and time, and stores them for further enrichment.
# MAGIC - upsert_heart_rate: Streams BPM data, filters invalid readings, and inserts unique readings per device and timestamp.
# MAGIC - age_bins: Categorizes users into age buckets based on date of birth. Used for downstream analytics.
# MAGIC - upsert_user_bins: Combines user demographics like age, gender, and location. Treated as a slowly changing dimension (SCD Type 1).
# MAGIC - upsert_completed_workouts: Joins start and stop workout events to identify valid sessions within a 3-hour window.
# MAGIC - upsert_workout_bpm: Joins completed workouts with heart rate data to create a time series of BPM during workout sessions.
# MAGIC - upsert: Runs all the above streams in a defined order, divided into three stages for optimized processing.
# MAGIC - assert_count & validate: Used to test and confirm expected row counts in the Silver tables.
